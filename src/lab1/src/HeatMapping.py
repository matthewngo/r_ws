...
def main():
    ...
    print 'getting scan'
    # get LIDAR scan
    bag = rosbag.Bag('/home/mvn3/r_ws/src/lab1/bags/laser_scans/laser_scan3.bag', 'r')
    lidar_ranges = []
    lidar_angles = []
    ...
    # set up angle matrix as column-major (x,y)
    angle_matrix = np.zeros((map_img.width, map_img.height))
    angle_matrix[:][:] = -1

    last_particle = particles[0]
    max_weight = weights[0]
    max_angle = particles[0][2]
    
    # get smallest weight in weight array
    ...
    np.savetxt('/home/mvn3/r_ws/bag3_angles.csv', angle_matrix, delimiter=',') 
    map_img.show()
